...
// 	return /(\b\w+\.?\w+\@\w+\.\w{2,3})/i.test(email);
// }
...
// 	var values = (/([^\[]*)\[([^\]]*)\]\(([^\)]*(\.com|\.io)\/?)\)(.*)/g).exec(text);

// 	if (!values) {
// 		return text;
// 	}

// 	var result = values[1] + '<a href="' + values[3] + '">' + values[2] + '</a>' + values[5];
// 	return result;
// }
...
// 	var result = values[2] + "<em>" + values[3] + "</em>" + values[4];
// 	console.log(result);
// 	return result;
// }
...
function extractPhoneNumber (text) {
	var result = text.match(/\(?\d{3}\)?[\s\-\.]?\d{3}[\s\.\-]?\d{4}/g);
	return result;
}
...
// 	var result = text.match(/[^ @]+@[^ \.]+\.(com|org|net)/g);
// 	return result;
// }
